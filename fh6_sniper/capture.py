    ...
    if raw is None:
        try:
            ...
            if not _capture_failing:
                _log.warning("Aufnahme fehlgeschlagen: %s", e)
                _capture_failing = True
            return False

    if _capture_failing:
        _log.info("Aufnahme wiederhergestellt")
        _capture_failing = False
...
# ── Aufnahme-Worker im Hintergrund ───────────────────────────────────────────
def _worker_alive() -> bool:
    return _worker_thread is not None and _worker_thread.is_alive()


def _worker_run(window_title):
    _log.info("Capture-Worker gestartet")
    while not _stop_worker:
        try:
            produced = _produce_frame(window_title)
        except Exception:
            _log.exception("Fehler im Capture-Worker")
            time.sleep(0.05)
            continue
        if not produced:
            time.sleep(0.003)
    _log.info("Capture-Worker gestoppt")
