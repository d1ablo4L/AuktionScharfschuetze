...
class GameIO:
    ...
    def screen(self, targets=None) -> Screen:
        ...
        if result != self._last_screen:
            log.info("Bildschirm -> %s", result.name)
    ...
    def press(self, name: str, times: int = 1) -> None:
        log.info("drücke %s%s", name, f" x{times}" if times > 1 else "")
...
class Sniper:
    ...
    def _guard_focus(self) -> None:
        if self.io.focused():
            return
        self._status("Pausiert: FH6 nicht im Vordergrund")
    ...
    def wait_for(self, screens: set, timeout: float):
        ...
        while self.clock() < deadline:
            ...
            if current in screens:
                log.info("warte %s -> %s", _names(screens), current.name)
                return current
            self._poll_delay()
        log.info("warte %s -> TIMEOUT nach %.0fs", _names(screens), timeout)
        return None
    ...
    def _goto_search_config(self) -> bool:
        ...
        self._status("Verloren: Starte den Bot im Auktionshaus")
        return False

    def _enter_search_from_landing(self, known=None) -> bool:
        self._status("Öffne Auktionssuche")
        for attempt in range(1, 5):
            if self._stop:
                return False
            s = known if known is not None else self.io.screen()
            known = None
            log.info("suche_öffnen Versuch %d: Bildschirm=%s", attempt, s.name)
            if s == Screen.SEARCH_CONFIG:
                return True
            if s == Screen.UNKNOWN:
                self.sleeper(0.6)
            ...
            if self.wait_for({Screen.SEARCH_CONFIG}, 0.9) is not None:
                return True
        log.info("suche_öffnen: nach 4 Versuchen abgebrochen")
        return False
    ...
    def _jitter_max_bid(self) -> None:
        ...
        try:
            self._status(f"Aktualisiere Gebot ({sign}{steps})")
            for _ in range(rows):
                if self._stop:
                    return
                self._press("up")
            ...
            log.exception(
                "Jitter übersprungen: Tasten 'up'/'left'/'right' nicht verfügbar")
            try:
    ...
    def _recover(self) -> str:
        ...
        for _ in range(14):
            ...
            if s in (Screen.SEARCH_CONFIG, Screen.AH_LANDING):
                log.info("Wiederherstellung: erreicht %s", s.name)
                return "recovered"
    ...
    def _escape_player_options(self) -> str:
        self._status("Angebot bereits verkauft, überspringe")
        for _ in range(6):
            if self._stop:
                return "recover_failed"
    ...
    def _collect(self) -> bool:
        self._status("Auto wird abgeholt")
    ...
    def run_once(self) -> str:
        ...
        if self._stop:
            return self._recover()
        t_query = self.clock()
        log.info("Zeit: Suche öffnen %.0f ms", (t_query - t_start) * 1000)
        result = self._press_until(
            "enter", Screen.SEARCH_CONFIG,
            {Screen.RESULTS_HAS_CARS, Screen.RESULTS_EMPTY})
        log.info("Zeit: Spiel-Abfrage %.0f ms", (self.clock() - t_query) * 1000)
        ...
        if slot == 0:
            self._status("Alle Angebote verkauft, überspringe")
            self._back_to_landing(known=result)
            return "no_cars"

        self._status("Auto gefunden, Sofortkauf")
        t_buy = self.clock()
        for _ in range(slot - 1):
            self._press("down")

        if slot > 1 and self.io.first_buyable_slot() != slot:
            self._status("Angebot während der Navigation verkauft, überspringe")
            self._back_to_landing(known=result)
            return "no_cars"
        ...
        log.info("Zeit: Sofortkauf %.0f ms", (self.clock() - t_buy) * 1000)
        self._press("enter")
        ...
        if cfg.collect_after_buyout:
            ...
            for _attempt in range(3):
                ...
                if self.io.screen() not in _claimable:
                    break
            log.info("Zeit: Abholung %.0f ms", (self.clock() - t_coll) * 1000)
        self._back_to_landing()
        return "bought"
    ...
    def run(self) -> str:
        ...
        log.info("=== Sniper gestartet ===")
        self._status("Läuft")
        while not self._stop:
            if self._auto_stop_reached():
                self._status("Auto-Stopp-Grenze erreicht")
                return "auto_stop"
            self._guard_focus()
            if self._stop:
                break
            ...
            log.info("Ausgang run_once: %s", outcome)
            self.searches += 1
            if outcome == "recover_failed":
                self._emit_stats()
                self._status("Gestoppt: Wiederherstellung nicht möglich")
                return "recover_failed"
            if outcome == "failed":
            ...
            if outcome == "bought":
                ...
                self._status(f"{self.cars_bought} Auto(s) gekauft")
                if self.on_purchase:
