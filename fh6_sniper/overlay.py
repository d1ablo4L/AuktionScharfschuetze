...
# Versuch: Schaltflächen in Kartenfarbe mit ROG-Schrift
_BTN_BG   = _CARD
_BTN_HV   = "#2b0005"
_BTN_FG   = _ROG

# Farben der deutschen Flagge (Titel + Streifen oben)
_DE_BLACK = "#1a1a1a"
_DE_RED   = "#dd0000"
_DE_GOLD  = "#ffce00"
_DE_WHITE = "#ffffff"
...
# ── Spezifikation des Einstellungsmenüs ──────────────────────────────────────────
SETTINGS_SPEC = [
    ("section", "Geschwindigkeit"),
    {"key": "match_threshold", "label": "Erkennungsschwelle", "kind": "slider",
     "lo": 0.50, "hi": 0.95, "step": 0.01, "int": False,
     "desc": "Wie sehr ein Bildschirm der Vorlage ähneln muss. Höher = strenger."},
    {"key": "poll_interval_ms", "label": "Prüfintervall (ms)", "kind": "range",
     "lo": 5, "hi": 150, "step": 1, "int": True,
     "desc": "Wie oft der Bildschirm geprüft wird. Niedriger = reagiert schneller."},
    {"key": "key_hold_ms", "label": "Tastendruckdauer (ms)", "kind": "range",
     "lo": 5, "hi": 80, "step": 1, "int": True,
     "desc": "Dauer jedes Tastendrucks. Niedriger = schneller (zu niedrig verliert Tastendrücke)."},
    {"key": "between_keys_ms", "label": "Pause zwischen Tasten (ms)", "kind": "range",
     "lo": 5, "hi": 120, "step": 1, "int": True,
     "desc": "Pause nach jeder Taste. Niedriger = schnellere Navigation."},
    {"key": "loop_pace_s", "label": "Pause zwischen Durchläufen (s)", "kind": "slider",
     "lo": 0.0, "hi": 1.0, "step": 0.01, "int": False,
     "desc": "Pause zwischen den Suchläufen. Niedriger = mehr Versuche pro Minute."},
    {"key": "buyout_select_delay_ms", "label": "Bestätigungsverzögerung (ms)", "kind": "slider",
     "lo": 0, "hi": 500, "step": 5, "int": True,
     "desc": "Zusätzliche Wartezeit vor dem Bestätigen des Kaufs. 0 = maximale Geschwindigkeit."},

    ("section", "Auto-Stopp"),
    {"key": "auto_stop_enabled", "label": "Auto-Stopp", "kind": "toggle",
     "desc": "Stoppt den Bot, wenn die unten stehenden Grenzwerte erreicht werden."},
    {"key": "max_cars", "label": "Zu kaufende Autos", "kind": "slider",
     "lo": 1, "hi": 100, "step": 1, "int": True,
     "desc": "Wie viele Autos gekauft werden, bevor gestoppt wird."},
    {"key": "max_minutes", "label": "Maximale Laufzeit (Min.)", "kind": "slider",
     "lo": 1, "hi": 600, "step": 1, "int": False,
     "desc": "Maximale Laufzeit in Minuten vor dem Stopp."},

    ("section", "Verhalten"),
    {"key": "jitter_maxbid", "label": "Höchstgebot aktualisieren", "kind": "toggle",
     "exclusive_group": "jitter_field",
     "desc": "Vor jeder Suche wird das Höchstgebot um ±1 verschoben (geht 2 Zeilen nach oben), um die Liste zu aktualisieren und bereits verkaufte Autos zu vermeiden. Beim Aktivieren wird der Höchstkaufpreis deaktiviert."},
    {"key": "jitter_maxbuyout", "label": "Höchstkaufpreis aktualisieren", "kind": "toggle",
     "exclusive_group": "jitter_field",
     "desc": "Wie oben, wirkt aber auf den Höchstkaufpreis (geht 1 Zeile nach oben). Beim Aktivieren wird das Höchstgebot deaktiviert. Sind beide aus, wird der Preis nicht aktualisiert."},
    {"key": "collect_after_buyout", "label": "Nach dem Kauf abholen", "kind": "toggle",
     "desc": "Holt das gekaufte Auto sofort ab (langsamer, aber automatisch)."},
    {"key": "notify_sound", "label": "Benachrichtigungston", "kind": "toggle",
     "desc": "Windows-Signalton bei jedem erfolgreichen Kauf."},
    {"key": "notify_toast", "label": "Windows-Benachrichtigung", "kind": "toggle",
     "desc": "Windows-Toast-Hinweis bei jedem Kauf."},
    {"key": "overlay_capturable", "label": "Overlay in Aufnahmen anzeigen", "kind": "toggle",
     "desc": "Nur zum Erstellen von Screenshots aktivieren: Damit wird das Overlay in Aufnahmen/Screenshots sichtbar, kann aber die Bildschirmbereiche verdecken, die das Tool zum Funktionieren ausliest. Lass es ausgeschaltet, während der Sniper läuft."},

    ("section", "Sicherheit"),
    {"key": "timeout_results_s", "label": "Timeout Ergebnisse (s)", "kind": "slider",
     "lo": 2, "hi": 30, "step": 0.5, "int": False,
     "desc": "Maximale Wartezeit auf die Ergebnisse nach einer Suche."},
    {"key": "timeout_outcome_s", "label": "Timeout Ausgang (s)", "kind": "slider",
     "lo": 5, "hi": 60, "step": 1, "int": False,
     "desc": "Maximale Wartezeit auf den Ausgang des Kaufs."},
    {"key": "timeout_claim_s", "label": "Timeout Abholung (s)", "kind": "slider",
     "lo": 5, "hi": 60, "step": 1, "int": False,
     "desc": "Maximale Wartezeit für die Abholung des Autos."},
    {"key": "timeout_generic_s", "label": "Allgemeiner Timeout (s)", "kind": "slider",
     "lo": 2, "hi": 30, "step": 0.5, "int": False,
     "desc": "Maximale Wartezeit für andere Bildschirmübergänge."},
    {"key": "buyout_confirm_window_s", "label": "Bestätigungsfenster Sofortkauf (s)", "kind": "slider",
     "lo": 0.0, "hi": 3.0, "step": 0.05, "int": False,
     "desc": "Wartezeit auf den Ausgang nach dem Bestätigungs-Enter."},
    {"key": "buyout_open_wait_s", "label": "Wartezeit Sofortkauf-Öffnung (s)", "kind": "slider",
     "lo": 0.0, "hi": 5.0, "step": 0.1, "int": False,
     "desc": "Wie lange auf das Öffnen des Sofortkauf-Bildschirms gewartet wird."},
    {"key": "collect_claim_wait_s", "label": "Abholung: Pause nach Bestätigung (s)", "kind": "slider",
     "lo": 0.0, "hi": 1.5, "step": 0.05, "int": False,
     "desc": "Pause nach dem Enter bei der Auto-Abholung. Niedriger = schnellere Abholung."},
    {"key": "collect_unknown_wait_s", "label": "Abholung: Übergangspause (s)", "kind": "slider",
     "lo": 0.0, "hi": 1.0, "step": 0.05, "int": False,
     "desc": "Pause, wenn der Bildschirm während der Abholung im Übergang ist."},

    ("section", "Tastenbelegung"),
    {"key": "hotkey_start_stop", "label": "Taste Start/Stopp", "kind": "text",
     "desc": "Globaler Hotkey, pynput-Format (z. B. <f8>). Neustart erforderlich."},
    {"key": "hotkey_panic", "label": "Notaus-Taste", "kind": "text",
     "desc": "Globaler Hotkey für sofortigen Stopp (z. B. <f9>). Neustart erforderlich."},
]


# ── Reine Hilfsfunktionen (ohne tkinter testbar) ─────────────────────────────────────
...
# ── Widget: animierte Statusleiste (gleitender Verlauf) ──────────────
class StateBar(tk.Canvas):
    ...
    def set_state(self, state):
        ...
        if state in ("running", "paused"):
            ...
            # Inaktiv: Leiste unsichtbar (verschwindet).
            self.delete("all")
            self._item = None
            self._photo = None
...
# ── Widget: abgerundete Pillen-Schaltfläche ────────────────────────────────────
...
# ── Widget: Vektorsymbol in einheitlicher Größe ───────────────────────────
def _hex_bgr(color):
    c = str(color).lstrip("#")
    if len(c) != 6:
        return None
...
# ── Gemeinsames Anti-Aliasing (cv2): scharfe Formen, keine Tk-Treppchen ───────
_AA_CACHE = {}
...
# ── Widget: Panel mit abgerundeten Ecken (nimmt andere Widgets auf) ─────────────
...
class Overlay:
    ...
    def __init__(self, cfg=None, on_save=None, hide_from_capture: bool = True):
        ...
        try:
            ...
            pass
        self.root.withdraw()
        self.root.title("AuktionScharfschütze - V.2")
        self.root.attributes("-topmost", True)
        self.root.overrideredirect(True)
        self.root.configure(bg=_BG)
        self._phase_var    = tk.StringVar(value="INAKTIV")
        self._status_var   = tk.StringVar(value="bereit zum Start")
    ...
    def _build(self):
        ...
        title_labels = []
        for chunk, color in (("Auktion", _DE_GOLD),
                             ("Scharf",  _DE_RED),
                             ("schütze", _DE_GOLD)):
        ...
        badge = tk.Canvas(header, width=bw, height=bh, bg=_BG_HDR,
                          highlightthickness=0, bd=0)
        # Hintergrund: Deutschland-Flagge (Schwarz / Rot / Gold)
        _band = bh / 3.0
        badge.create_rectangle(0, 0, bw, _band, fill=_DE_BLACK, outline="")
        badge.create_rectangle(0, _band, bw, 2 * _band, fill=_DE_RED,
                               outline="")
        badge.create_rectangle(0, 2 * _band, bw, bh, fill=_DE_GOLD,
                               outline="")
        # Ecken abrunden: Eck-Masken in der Header-Hintergrundfarbe
        _br = round(5 * UI_SHRINK)
        for _cx, _cy, _dx, _dy in ((0, 0, 1, 1), (bw, 0, -1, 1),
                                   (0, bh, 1, -1), (bw, bh, -1, -1)):
            badge.create_polygon(
                _cx, _cy, _cx + _dx * _br, _cy, _cx, _cy + _dy * _br,
                fill=_BG_HDR, outline="")
        # "V2" mit Schatten für bessere Lesbarkeit auf der Flagge
        badge.create_text(bw / 2 + 1, bh / 2 + 1, text="V2", fill="#000000",
        ...
        self._toast = tk.Label(
            self._header, text="", bg=_DE_GOLD, fg="#1a1a1a",
    ...
    def _build_status_page(self):
        ...
        cells = (
            ("GEKAUFT",   self._bought_var,   _DE_GOLD),
            ("SUCHEN",    self._searches_var, _ROG),
            ("FEHLER",    self._fails_var,    _RED_STAT),
            ("LAUFZEIT",  self._time_var,     _ROG),
        )
        ...
        tk.Label(footer, text="F8  Start / Stopp      \u00b7      F9  Notaus",
                 bg=_BG, fg=_DIM, font=("Segoe UI", 10)).pack()
        return page

    # ── Seite EINSTELLUNGEN (Unterabschnitte) ────────────────────────────────
    ...
    def _build_settings_page(self):
        ...
        if self._cfg is None:
            tk.Label(page, text="Einstellungen nicht verfügbar.", bg=_BG,
                     fg=_DIM, font=("Segoe UI", 9)).pack(padx=PAD, pady=PAD)
            return page

        tk.Label(page, text="Einstellungen", bg=_BG, fg=_ROG,
    ...
    def _show_toast(self, text="Gespeichert \u2713"):
    ...
    def _build_logs_page(self):
        ...
        self._page_title(page, "Log",
                         "Sicher zu teilen: keine Schlüssel oder persönlichen Daten.")
        ...
        mkbtn(bar, "KOPIEREN", self._copy_logs)
    ...
    def _render_logs(self):
        ...
        if not rows:
            self._log_text.insert("end", "  noch keine Ereignisse.\n")
        for ts, msg in rows:
            line = f"{ts}  {msg}"
    ...
    def _build_help_page(self):
        page = tk.Frame(self._holder, bg=_BG)
        self._page_title(page, "Hilfe", "Schnellstart und Fehlerbehebung.")
        blocks = [
            ("Schnellstart", [
                "1. Öffne das Spiel und gehe zu Auktionen suchen.",
                "2. Prüfe, ob der Status bereit ist.",
                "3. Drücke START oder die Taste F8.",
            ]),
            ("Wenn Autos verpasst werden",
             ["Verringere das Prüfintervall und die Pause zwischen Durchläufen "
              "in den Einstellungen. Ist die Erkennungsschwelle zu hoch, "
              "senke sie, bis die Bildschirme erkannt werden."]),
        ]
    ...
    def _build_about_page(self):
        ...
        tk.Label(page,
                 text="Dies ist eine an die deutsche Spielsprache angepasste Version des CecchinoDelleAste und hat nichts mit der kostenpflichtigen V2 von Frosty zu tun." \
                 " Wenn du Unterstützung brauchst oder Entwickler bist und zum Verbessern des Tools beitragen möchtest, habe ich einen eigenen Discord-Server erstellt:",
        ...
        PillButton(page, text="Discord beitreten", command=_open_discord,
        ...
        tk.Label(page, text="Version", bg=_BG, fg=_ROG,
                 font=("Segoe UI", 14, "bold")).pack(anchor="w", padx=PAD,
                                                     pady=(14, 2))
        tk.Label(page, text="V.2.0.0  \u00b7  Erstellt von d1ablo", bg=_BG,
                 fg=_DIM, font=("Segoe UI", 13)).pack(anchor="w", padx=PAD)
        return page

    # ── Seiten-Hilfsfunktionen ────────────────────────────────────────────────────────
    ...
    def _set_button(self, running: bool):
        if running:
            self._btn.set_mode("STOPP", _BTN_BG, _BTN_HV, _BTN_FG)
        else:
            self._btn.set_mode("START", _BTN_BG, _BTN_HV, _BTN_FG)

    # ── Status ──────────────────────────────────────────────────────────────
    def _retint(self):
        """Phase, Farben und animierte Leiste je nach echtem Status + Text."""
        detail = self._status_var.get().lower()
        if not self._running:
            phase, color, bar = "INAKTIV", _DIM, "idle"
        elif "pausiert" in detail:
            phase, color, bar = "PAUSIERT", _AMBER, "paused"
        else:
            phase, color, bar = "AKTIV", _ROG, "running"
