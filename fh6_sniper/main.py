...
def main() -> None:
    ...
    logging.getLogger("fh6").info("FH6 Sniper Start (Log: %s)", log_path)
    ...
    def on_purchase(loop_seconds, total):
        notifier.log_purchase(purchase_log, "gekauft", loop_seconds, total)
        ...
        def _run_safe():
            try:
                ...
                logging.getLogger("fh6.main").exception(
                    "Absturz des Sniper-Threads")
                try:
                    overlay.set_status("Absturz: siehe sniper.log")
                except Exception:
                    pass
            finally:
